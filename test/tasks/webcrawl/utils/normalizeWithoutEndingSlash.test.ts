	...
	describe('utils', () => {
		describe('normalizeWithoutEndingSlash', () => {
			it('should remove ending slash when there is no path', () => {
				expect(normalizeWithoutEndingSlash(URI_STRING_WITH_SLASH)).toBe(URI_STRING);
				expect(normalizeWithoutEndingSlash(URI_STRING)).toBe(URI_STRING);
			});
			it('should NOT add an ending slash when there is a path', () => {
				expect(normalizeWithoutEndingSlash(URI_STRING_WITH_PATH)).toBe(URI_STRING_WITH_PATH);
				expect(normalizeWithoutEndingSlash(URI_STRING_WITH_PATH_AND_SLASH)).toBe(URI_STRING_WITH_PATH);
			});
		});
	});
});
